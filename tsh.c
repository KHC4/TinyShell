...
/*Custom helpers*/
int rem_red(int argc, char **argv);
...
void eval(char *cmdline)
{
    ...
    if (!builtin_cmd(argv))
    {
        ...
        for (int t = 0; t < argc; t++)
        {
            if ((strcmp(argv[t], "<")) == 0)
            {
                if (!input_fd && (t + 1) < argc)
                {
                    input_fd = open(argv[t + 1], O_RDONLY, 0600);
                    if (input_fd == -1)
                    {
                        perror("open");
                        return;
                    }
                    argc = rem_red(argc, argv);
                }
            }
            else if ((strcmp(argv[t], ">")) == 0)
            {
                if (!output_fd && (t + 1) < argc)
                {
                    output_fd = open(argv[t + 1], O_WRONLY | O_CREAT, 0600);
                    if (output_fd == -1)
                    {
                        perror("open");
                        return;
        ...
        if (child == 0)
        {
            ...
            char cmdline_array[MAXARGS];
            strcpy(cmdline_array, cmdline);
            char *result = strtok(cmdline_array, "|");
            char *tokenized_argv[MAXARGS];
            int size = 0;
            while (result != NULL)
            {
                tokenized_argv[size] = result;
                result = strtok(NULL, "|");
                size++;
            }
            if (size == 1)
            {
                if (input_fd)
                {
                    dup2(input_fd, STDIN_FILENO); // set the input of the child to the file
                }

                if (output_fd)
                {
                    dup2(output_fd, STDOUT_FILENO); // set the output of the child to the file
                }
            ...
            else
            {
                ...
                {
                    char *argv[MAXARGS];
                    int argc = parseline(tokenized_argv[z], argv);
                    rem_red(argc, argv);
                    if (argv[0] == NULL)
                    {
                        return;
        ...
        {
            sigset_t siguser_set;
            sigemptyset(&siguser_set);
            sigaddset(&siguser_set, SIGUSR1);

            if (bg_task)
            {
                addjob(jobs, child, BG, cmdline);
                struct job_t *current = getjobpid(jobs, child);
                printf("[%d] (%d) %s", current->jid, current->pid, current->cmdline);

                sigprocmask(SIG_UNBLOCK, &blocked_signals, NULL);
                if (!ready){
                    sigsuspend(&siguser_set);
                }
...
void do_bgfg(char **argv)
{
    if (argv[1] != NULL)
    {
        ...
        if (strncmp(argv[1], "%%", 1) == 0)
        {
            char *jid_ptr = argv[1];
            jid_ptr++;
            int jid = (int)strtol(jid_ptr, NULL, 10);
        ...
        {
            long long int ll_pid = strtoll(argv[1], NULL, 10);
            pid_t pid = (pid_t)ll_pid;
            if (!pid)
            {
 ...
 * finds and removes the redirect symbol and file name from argv
 */
int rem_red(int argc, char **argv)
{
    int current = -1;
